    ...
    for (k in k_loops) {
      ...
      for (r in 1:reps) {
        for (i in 1:2) {
          ...
          a = system.time({
            lpca_no = logisticPCA(mat, k = k, m = 4, main_effects = FALSE, use_irlba = F)
          })
          b = system.time({
            lpca_yes = logisticPCA(mat, k = k, m = 4, main_effects = FALSE, use_irlba = T)
          })
...
ggplot(subset(results_m, Matrix_Type == "Low-Rank"), aes(cols, value, colour = irlba)) +
  geom_point() + geom_line() + facet_grid(rows ~ k) + 
  scale_x_log10() + scale_y_continuous(limits = c(NA, max_time)) + ggtitle("Low-Rank Matrix")

ggplot(subset(results_m, Matrix_Type == "Random"), aes(cols, value, colour = irlba)) +
  geom_point() + geom_line() + facet_grid(rows ~ k) + 
...
# ggplot(subset(results_m, Matrix_Type == "Low-Rank"), aes(cols, SpeedUp)) +
#   geom_point() + geom_line() + facet_grid(rows ~ k) + geom_hline(lty = 2, yintercept = 1) +
#   scale_x_log10() + scale_y_log10(limits = c(NA, max_time)) + ggtitle("Low-Rank Matrix")
# 
# ggplot(subset(results_m, Matrix_Type == "Random"), aes(cols, SpeedUp)) +
#   geom_point() + geom_line() + facet_grid(rows ~ k) + geom_hline(lty = 2, yintercept = 1) +
#   scale_x_log10() + scale_y_log10(limits = c(NA, max_time)) + ggtitle("Random Matrix")
